 ...
 *   **Reversed by SHELL-9 (2026-09-09, ADR-0050) — see below.** The
 ...
 *
 * SHELL-9 (2026-09-09, ADR-0050): this component is no longer render-only
 ...
 * (NFR-55). Two structural consequences worth knowing:
 *
 ...
 *   same commit (ADR-0050's own Consequences names this; `AppBreadcrumb.test.tsx`
...
/**
 * SHELL-9 (ADR-0050): resolved nav context passed alongside the route's own
 ...
 * which is exactly the partial state ADR-0050 §4 rules out.
 */
...
const ROUTE_BREADCRUMBS: RouteBreadcrumbConfig[] = [
  ...
  {
    // ADR-0070: the read-only detail view. `matchPath` is exact by default,
  ...
  // SHELL-9 (ADR-0050): all 5 project-scoped patterns below open with the
  ...
  {
    // ADR-0070: the project-scoped half of the same read-only detail view.
...
function AppBreadcrumb() {
  ...
  // SHELL-9 (ADR-0050): called unconditionally at the top of the body, before
