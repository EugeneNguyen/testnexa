...
**Addendum (2026-09-12, [ADR-0060](../adr/0060-projects-page-retired-generic-surface.md)):** the nav item, its position, and the dedicated-page destination above are all still exactly true — only the *component* rendering that page changed, from the bespoke `ProjectsPage` (ID/Name/Standards-profile/Actions table, "New Project" modal) this story originally shipped to the generic admin surface (Organization/Name/Standards-profile columns, "New" button, edit via a route not a modal). This ACs' own literal column-list/button-label wording is a historical record of what PROJ-4 delivered, not a current-state description — see the Sitemap doc's own `/orgs/:orgId/projects` row for the current shape.
