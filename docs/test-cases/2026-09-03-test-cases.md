...
(RBAC & Multi-Tenancy's count was corrected from a stale 19/13 to the actual 23/16 pre-RBAC-3 rows — TC-RBAC-020..023 had been added by RBAC-1 without this table being updated — plus RBAC-3's own 16 new rows: TC-RBAC-024..035 (API) and TC-RBAC-036..039 (the `GET /orgs/{org_id}/roles` endpoint + Role Assignments UI, added on top of the original API-only scope per user direction). Project & Release and Layout & Navigation rows are PROJ-2/SHELL-1..4's own counts, merged in unchanged.)
