...
| TC-ADMIN-014 | Field-type-driven input rendering, per type | Entity config with a `string`/`enum`/`fk`/`date`/`boolean` field each | Open the create form | Each field renders its matching CoreUI input (`FormField`/`CFormSelect`/`FkAutocomplete`/date input/`CFormSwitch`) | P2 | ADMIN-2 (ADR-0027) |
