...
| TC-SHELL-010 | Dashboard widgets show real, seeded counts | Org with N projects, M active members (seeded fixture) | Load `/orgs/:orgId` | Project widget shows N, Org Member widget shows M — matches fixture, not a hardcoded value | P1 | ADR-0020 (FR-SHELL-3, NFR-27) — **interim status:** `GET /projects`/`GET /org-memberships` ([ADR-0021](../adr/0021-generic-crud-router-factory.md)'s generic-CRUD factory) are designed (this doc batch) but not yet implemented, so this exact scenario isn't achievable today; unit-tested against a mocked resolved count (`OrgHome.widgets.test.tsx`), and E2E currently asserts the honest-error-state fallback instead (`shell-full-template.spec.ts`) — rewrite to real seeded counts once ADR-0021's factory is implemented |
