...
| TC-ADMIN-014 | Field-type-driven input rendering, per type | Entity config with a `string`/`enum`/`fk`/`date`/`boolean` field each | Open the create form | Each field renders its matching CoreUI input (`FormField`/`CFormSelect`/`FkAutocomplete`/date input/`CFormSwitch`) | P2 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-015 | Create button absent (not disabled) without `.create` permission | User lacks `environment.create` | Open the Environment admin list | "New" button is absent from the DOM entirely | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-016 | Read-only entity has no write actions for anyone | Any actor, any permission set | Open the Permission/TestLog admin list | No "New"/Edit/Delete affordance renders regardless of the actor's own grants — config has no `create`/`update`/`delete` at all | P2 | ADMIN-2 (ADR-0025) |
...
| TC-ADMIN-018 | RiskItem scope-selector blocks the list query until resolved | No Requirement/TestPlan selected yet on the RiskItem admin page | Load the page, inspect network calls | Zero list requests fire; selecting a Requirement fires exactly one request with `?requirement_id=<id>` | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-019 | Attachment scope-selector resolves via TestCase | No TestCase selected yet on the Attachment admin page | Select a TestCase | List request fires scoped to that TestCase's resolved chain, matching `resolve_via_test_case` | P2 | ADMIN-2 (ADR-0025) |
...
| TC-ADMIN-021 | `permissions/mine` with zero grants is 200-empty, not 403 | Actor is a member of `org_id` with no `RoleAssignment` at all | Call the endpoint | 200, `{"codes": []}` — membership alone is sufficient, no permission code is required to call this route | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-022 | `permissions/mine` 404s for a non-member | Actor has zero `OrgMembership` in `org_id` | Call the endpoint | 404, same NFR-1 boundary as every other org-scoped route | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-023 | TestExecution generic CRUD, one-hop resolver | TestExecution seeded in Org A | Actor in Org B: GET `/test-executions/{id}` | 404 — proves the new `TestCycle`-hop resolver enforces tenant isolation | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-024 | TestLog list/get only, no write route exists | — | Attempt `PATCH`/`DELETE /test-logs/{id}` | No such route is registered (405/404 at the routing layer, not an application-level rejection) — immutability enforced structurally | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-025 | Traceability link tables list/get only, cross-org 404 | `RequirementTestCaseLink` seeded in Org A | Actor in Org B: GET `/requirement-test-case-links/{id}` | 404; separately, `PATCH`/`DELETE` on the same id is not a registered route for any of the 4 link tables | P1 | ADMIN-2 (ADR-0025) |
| TC-ADMIN-026 | Generic admin surface coexists with a bespoke screen | `Organization`/`Project`/`OrgMembership`/`Release` each have both a bespoke screen and a generic admin page | Delete an `OrgMembership` row via the generic admin page (no such action exists on `OrgMembers.tsx`) | Row deleted via the generic surface; the bespoke screen is unaffected/unaware, no duplicate-action confusion in either UI | P3 | ADMIN-2 (ADR-0025) |
