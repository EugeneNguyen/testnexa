...
**Date:** 2026-09-06
**Status:** Accepted (MCP-1 ships the direct-link path only — see "Drift" note below; rigor-path twin + MCP-2/MCP-3 backlog — **superseded by [ADR-0065](0065-mcp-5-full-crud-all-entities.md), 2026-09-14**, which subsumes MCP-2/MCP-3's own hand-wired-tool backlog into a registry-driven generic tool surface instead — see that ADR's Drift note. **MCP-1's own two shipped tools, `create_test_case`/`list_test_cases`, are themselves retired by [ADR-0068](0068-mcp-6-per-entity-mcp-tools.md), 2026-09-13** — folded into that ADR's generated per-entity naming scheme as `tn_test_case_create`/`tn_test_case_list`, which also closes this ADR's own Drift note by supporting REQ-3's rigor path the hand-wired tool never did. Decision 2's direct-call dispatch and decision 4's error-envelope contract are unchanged and still govern.)
