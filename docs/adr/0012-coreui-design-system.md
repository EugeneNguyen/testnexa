...
**Date:** 2026-09-03
**Status:** Accepted (Partially superseded by [ADR-0036](0036-shell-components-raw-html-not-coreui-react.md) for the 5 shell-chrome files only — CSS/design-token layer unaffected, "build UI from CoreUI's React components first" clause lifted for `AppShell`/`AppHeader`/`AppSidebar`/`AppFooter`/`AppBreadcrumb`)
