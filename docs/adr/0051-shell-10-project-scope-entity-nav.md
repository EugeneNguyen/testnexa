...
- **This is an amendment to ADR-0050's Decision §2, not a reversal of the whole ADR.** SHELL-9's own `useResolvedOrgId()` hook (aside from the new `mode` field), its breadcrumb work, and its own 6 TCs are entirely unaffected and still accurate — only the *sidebar content* SHELL-9 chose to render on project routes changes here. Per this repo's own "flag drift, don't silently absorb" convention, ADR-0050's own text is left as-written; this ADR is the record of what changed and why.
