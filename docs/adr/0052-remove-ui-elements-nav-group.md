...
- new ADR-0052 (this file), supersedes ADR-0020's UI Elements clause (ADR-0020's `Status` row edited to read "Partially superseded by 0042, 0052")
- ADR README: new row + Date line extended
- WBS row `11.35`
- Test Design `§43`
- Test Cases: TC-SHELL-014 ~~strikethrough~~ + one-line "superseded by ADR-0052" note; TC-SHELL-027 corrected in place (7-item expected sequence → 6-item, "UI Elements" trailing entry removed); new TC-SHELL-039 (P2: sidebar's `UI Elements` group absent under `/orgs/:orgId`, the deletion itself asserted as an explicit negative, not inferred from TC-SHELL-027's now-6-item order alone); Layout & Navigation coverage-summary recount note (22/9 stays 22/9 — strike `-1` P3 + add `+1` P2)
