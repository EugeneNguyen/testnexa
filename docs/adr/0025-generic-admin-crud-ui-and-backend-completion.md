...
- **`TestExecution`** (full CRUD): `app/api/routes/execution.py`, `resolve_org_id = chain_resolver([(TestCycle, "test_cycle_id")])` → `TestCycle.project_id` → `Project.org_id`. New `schemas/execution.py` entries (`CreateTestExecutionRequest`, `UpdateTestExecutionRequest`, `TestExecutionSummary`).
