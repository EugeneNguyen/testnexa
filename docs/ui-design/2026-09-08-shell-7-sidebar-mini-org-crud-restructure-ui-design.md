...
**Sources:** [ADR-0044](../adr/0044-shell-7-sidebar-mini-org-crud-restructure.md), [SHELL-7 user story](../user-stories/2026-09-04-admin-shell-sidebar-stories.md#story-shell-7-mini-sidebar--org-scoped-crud-nav-restructure), [ADR-0042](../adr/0042-adminlte-design-system.md) (AdminLTE v4, `sidebar-mini`'s shipped CSS), [ADR-0043](../adr/0043-atomic-design-tiering-for-frontend-components.md) (`AppSidebar`/`AppShell` current locations)

## 1. Scope

Two changes to the existing shell, no new screens: (a) `AppShell` gains AdminLTE's `sidebar-mini` layout modifier; (b) `AppSidebar`'s org-scoped `Admin` group (8 entities, one flat list) becomes 3 named groups. Frontend-only, no backend/schema change (see ADR-0044).
