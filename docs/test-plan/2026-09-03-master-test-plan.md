...
- FR-ADMIN-2 ([ADR-0021](../adr/0021-generic-crud-router-factory.md)) is release-blocking across five distinct assertions, not just "the factory's happy path works": (a) at least one representative entity per resolver depth (direct, one-hop, branching `RiskItem`, multi-hop `TestCase`/`Defect`, global-catalog) has its cross-tenant 404 boundary integration-tested, not just the direct-scope case generalized to all 20 entities untested; (b) `DELETE` on a RESTRICT-referenced row returns `409`, tested as distinct from `422` (NFR-29); (c) `RiskItem` create with both `requirement_id` and `test_plan_id` set → `422`, not silently accepted (the DB `CHECK` alone would allow it); (d) `Role.org_id IS NULL` → `404` on `PATCH`/`DELETE` but `200` on `GET` (NFR-31) — the read/write split is easy to under-implement as "404 everywhere" or "404 nowhere"; (e) an entity's `?q=` search is proven to actually filter (not silently ignored) when `search_fields` is configured, and proven to silently no-op (not `422`) when it isn't (NFR-30). This closes TC-SHELL-010's previously-interim status (dashboard widgets calling `GET /projects`/`GET /org-memberships`, currently 404) once the factory itself ships — that rewrite is tracked in Test Cases, not repeated here.
...
| `DELETE`'s `409`-vs-`422` distinction (NFR-29) is easy to skip if `delete` reuses `create`/`update`'s existing flush-then-catch-`IntegrityError`→`422` handler verbatim instead of a dedicated `409` branch | Dedicated test per representative entity asserts `409` specifically (not just "delete failed with some 4xx"), so a copy-pasted error handler fails immediately |
