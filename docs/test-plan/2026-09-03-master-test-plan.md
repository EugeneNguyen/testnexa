...
- **DS-1's AC3 (behavior-preserving migration) is release-blocking for DS-1 itself, not just a nice-to-have regression check**: `Login.tsx`'s existing Playwright coverage (`auth-login.spec.ts`) and `Signup.tsx`'s existing Vitest (`Signup.test.tsx`, `Signup.authFlow.test.tsx`) and Playwright (`auth-signup.spec.ts`) coverage must all pass **unmodified in their assertions** after both screens migrate onto `FormField` + React Hook Form + Zod — a passing suite that required rewriting assertions to match new behavior would not prove `FormField` is a true drop-in, it would just prove the tests were changed to match whatever shipped. NFR-29 (`CFormFeedback`+`invalid` error-display convention) is verified by `FormField`'s own unit tests (TC-DS-003/TC-DS-004), not by inspection alone.
