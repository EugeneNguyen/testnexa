...
- NFR-25 (FR-SHELL-3 dashboard widgets, honest-data-only) is release-blocking for the widget deliverable: a widget count must be asserted against a real seeded fixture value (integration/E2E), never asserted only against a hardcoded/mocked number in a way that would pass even if the underlying query were wrong. NFR-26 (FR-SHELL-4 dark-mode `localStorage`-only persistence) is verified by an E2E test that reloads the page and asserts the theme survives (TC-SHELL-006) — a toggle that only changes in-memory state without persisting does not satisfy FR-SHELL-4.
...
| FR-SHELL-3 dashboard widgets could silently drift into "decorative" numbers if a future refactor swaps the data source without updating the test fixture (e.g. widget count stops reflecting real `total`) | Widget tests assert against a seeded, known fixture count (not a snapshot of whatever the DB happens to contain), so a data-source regression changes the asserted number and fails the test, per NFR-25 |
| UI-element reference pages (Colors/Typography/Icons, ADR-0019) have no FR/story behind them — risk they get silently expanded into real feature surface over time (scope creep, contradicting their own documented "scaffolding only" status) | No acceptance-criteria-style tests are written for their *content*, only smoke-level render tests (they exist, sidebar links reach them); any future PR adding business logic to them should be rejected at review as scope creep unless a story is written first |
