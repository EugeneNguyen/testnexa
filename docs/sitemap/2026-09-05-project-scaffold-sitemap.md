...
| `/projects/:projectId` | `ProjectDetail` | Release list, per-release TestCycle/TestExecution audit view (PROJ-2); Requirement list + "New Requirement" modal (REQ-1); per-Requirement direct-link TestCase list + "New Test Case" modal, per-TestCase TestStep list + add/inline-edit (REQ-2, ADR-0006/[ADR-0029](../adr/0029-testcase-resolver-direct-link-fallback.md)); per-Requirement Test Condition list + create, per-Test-Condition Test Case create + list (REQ-3/REQ-4, [ADR-0028](../adr/0028-req3-test-condition-rigor-path-bespoke-routes.md)) — the two coexist per-TestCase within a project (ADR-0006); Test Suites list + create modal, per-suite live TestCase-membership view with add/remove (REQ-4, [ADR-0030](../adr/0030-req4-test-suite-membership-bespoke-routes.md), [REQ-4 UI Design Document](../ui-design/2026-09-06-req-4-test-suite-membership-ui-design.md)) |
