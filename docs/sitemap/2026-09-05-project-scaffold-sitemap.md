...
**MCP-4** ([ADR-0063](../adr/0063-mcp-integration-screen.md), FR-MCP-4) — the exception to MCP-1's own "no Sitemap impact" note above: this story adds the human-facing *frontend* screen MCP-1..3 never got, so it does add 2 real routes (below). The MCP server endpoint itself (`POST /mcp`) is still out of scope for the same reason MCP-1's note gives.
...
**ADMIN-4** ([ADR-0056](../adr/0056-admin-4-generic-admin-crud-column-sort.md), FR-ADMIN-3/NFR-60) — no new route, no removed route, no changed screen. Column sort is a click-header interaction and a `?sort=` query param on every existing generic-admin `:entity` list route already in this table — no `App.tsx` change, no new/renamed route.
...
| `/orgs/:orgId/projects` | `EntityListPage` (`entityKeyOverride="projects"`) | **[ADR-0047](../adr/0047-proj-4-projects-page-sidebar-entry.md) (PROJ-4) established this route/nav item; [ADR-0060](../adr/0060-projects-page-retired-generic-surface.md) retired its bespoke `ProjectsPage` component in favor of the generic admin surface, same URL/nav item.** List/Create/Edit/Delete via the generic surface (Organization/Name/Standards profile columns, sortable; server-side `?q=` name search; row's own name links to `ProjectDetail`, `detailPath`/`detailLinkField`) — reached via `AppSidebar`'s "Projects" nav item, or the Dashboard's Project-count widget link |
| `/orgs/:orgId/projects/:id/edit` | `EntityFormPage` (`entityKeyOverride="projects"`) | **New ([ADR-0060](../adr/0060-projects-page-retired-generic-surface.md)):** `ProjectsPage`'s edit was a modal, no route — the generic surface's edit is always a route |
| `/orgs/:orgId/members` | `OrgMembers` | RBAC-2 |
| `/orgs/:orgId/mcp` | `McpIntegration` | **New ([ADR-0063](../adr/0063-mcp-integration-screen.md), FR-MCP-4):** MCP server explanation + Claude Code/Codex CLI/Cursor/generic-client connection docs, plus live `AIAgent` credential issue/list/revoke (org-scoped, ADR-0015) — reached via `AppSidebar`'s "MCP Integration" nav item. One shared component with the project-scoped row below (mode resolved via `useResolvedOrgId()`, no `scope` prop) |
...
| `/projects/:projectId/mcp` | `McpIntegration` | **New ([ADR-0063](../adr/0063-mcp-integration-screen.md), FR-MCP-4):** same shared component as `/orgs/:orgId/mcp` above — project mode renders connection docs only (no key-management panel; `AIAgent` credentials are org-scoped) plus a link back to the org-scoped screen |
...
├── /projects                            EntityListPage, entityKeyOverride="projects" (PROJ-4 route; ADR-0060 retired the bespoke ProjectsPage behind it)
│   └── /projects/:id/edit               EntityFormPage, entityKeyOverride="projects" (ADR-0060 — ProjectsPage's edit was a modal, no route)
├── /members                             OrgMembers
