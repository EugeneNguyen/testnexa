...
`POST /orgs/{org_id}/agents` (AUTH-4, [ADR-0014](../adr/0014-ai-agent-credential-mechanics.md)): request `{agent_name, model_or_provider?, acting_on_behalf_of_user_id}`. Requires a human `User` bearer token (hardcoded check, independent of `RoleAssignment` — an `AIAgent` bearer credential calling this route gets `403 actor_forbidden` regardless of any permission it might hold, NFR-14) **and** `require_permission("ai_agent.create")` scoped to `org_id`. `acting_on_behalf_of_user_id` must resolve to an active `OrgMembership` in `org_id`, or `422`. On success: creates `Actor(actor_type=ai_agent)` + `AIAgent` row (`issued_at=now`, `revoked_at=NULL`, `last_used_at=NULL`), generates the raw key, and returns `{agent_id, agent_name, api_key, key_prefix}` — `api_key` is the full raw `tnx_agent_...` value, shown exactly once, never retrievable again (only `AIAgent.key_hash` is persisted). Caller with zero `OrgMembership` in `org_id` → `404` (NFR-16); membership present but missing `ai_agent.create` → `403`.
...
```
Same `actor_forbidden` shape is reused verbatim for an `AIAgent` bearer credential calling `POST /orgs/{org_id}/agents` or `.../revoke` (NFR-14, [ADR-0014](../adr/0014-ai-agent-credential-mechanics.md)) — one human-only-gate error code across both enforcement points, not a route-specific variant.
