...
| GET | `/test-logs?test_execution_id=<uuid>` | `test_log.read` | generic factory list ([ADR-0025](../adr/0025-generic-admin-crud-ui-and-backend-completion.md)) — same permission code, paginated/filterable equivalent of the bespoke route above |
| GET | `/test-logs/{id}` | `test_log.read` | generic factory item route |
| GET | `/requirement-test-case-links?requirement_id=<uuid>` | `requirement_test_case_link.read` | generic factory list ([ADR-0025](../adr/0025-generic-admin-crud-ui-and-backend-completion.md)); row created only as a side effect of `POST /requirements/{id}/test-cases` (§4) |
