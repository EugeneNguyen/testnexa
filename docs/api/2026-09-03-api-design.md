...
| GET | `/executions/{id}/logs` | `test_execution.read` | FR-EXEC-2 AC3 — ordered TestLog timeline, read-only, built, [ADR-0036](../adr/0036-exec-2-append-only-test-log.md) |
| POST | `/executions/{id}/comments` | `test_execution.update` | FR-EXEC-2 AC1 — appends a TestLog row (`comment`/`attachment`), no separate Comment entity, built, [ADR-0036](../adr/0036-exec-2-append-only-test-log.md) |
...
| GET | `/executions/{id}/logs` | `test_execution.read` | bespoke ordered timeline (§4) — gated `test_execution.read`, not `test_log.read`, correcting this row's own earlier (pre-build) text, [ADR-0036](../adr/0036-exec-2-append-only-test-log.md) |
