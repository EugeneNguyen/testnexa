...
**DASH-3** ([ADR-0063](../adr/0063-dash-3-dashboard-org-list-and-chooser.md), FR-DASH-3/FR-DASH-4) — reviewed, no API impact. `Dashboard` becomes a second caller of the already-documented `GET /auth/me/orgs` (§2, SHELL-6/ADR-0036) — no new route, no changed request/response shape, no changed permission gate. Retiring `/orgs/pick`/`OrgPicker.tsx` removes a frontend route/component only; it called no backend route of its own to begin with.
