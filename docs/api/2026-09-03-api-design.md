...
| POST | `/executions/{id}/defects` | `defect.create` | FR-EXEC-3 — creates Defect + TestCaseDefectLink, `422` if execution `result != fail`, built, [ADR-0041](../adr/0041-exec-3-raise-defect-from-execution.md) |
| GET | `/test-cases/{id}/defects` | `defect.read` | FR-EXEC-3 AC3 — every Defect ever raised against any of this TestCase's executions, ordered `created_at` descending, not in this document's original scope; added by [ADR-0041](../adr/0041-exec-3-raise-defect-from-execution.md) since AC3's "most recent first" has nothing safely ordered to call without it |
