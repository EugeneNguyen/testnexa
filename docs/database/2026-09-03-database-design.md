...
**SHELL-7** ([ADR-0044](../adr/0044-shell-7-sidebar-mini-org-crud-restructure.md), FR-SHELL-7) — reviewed, no schema impact. The sidebar-mini layout is a body-class CSS toggle; the org-scoped CRUD nav restructure only regroups `AppSidebar.tsx`'s presentation of the existing `orgScopedEntities` registry (`Role`, `Permission`, `RoleAssignment`, `OrgMembership`, `TestDesignTechnique`, `TestLevel`, `TestType`, `Organization`) into 3 named groups — no table, column, index, or route reads/writes any differently. Noted here explicitly so the gap isn't mistaken for an oversight.
