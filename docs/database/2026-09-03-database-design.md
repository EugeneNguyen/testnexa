...
| `test_manager` | `test_plan.*` + `.approve`, `entry_exit_criteria.*`, `test_cycle.*`, `test_suite.*`, `release.create`/`.read`/`.update`, `environment.*`, `test_execution.create`/`.read`, `project.read`/`.update` (RBAC-3/[ADR-0021](../adr/0021-role-assignment-creation-flow.md) — closes a gap that made TC-RBAC-035's regression case unprovable: a project's own creator, auto-granted this Role project-scoped by PROJ-1, couldn't otherwise view or rename the project itself), `approval.create`/`.read`, `requirement.read`/`.export_rtm`, `defect.read`, `risk_item.*`, `test_case.read`, `test_step.read`, `test_condition.read`, and (2026-09-15, [ADR-0075](../adr/0075-junction-table-registry-completeness.md)) `test_suite_test_case.read`/`test_plan_test_suite.read` — this role already held full `test_suite`/`test_plan` CRUD, so without the two junction codes the ADR-0074 relationship tabs on those screens would `403` |
| `tester` | `test_case.*`, `test_step.*`, `test_condition.*`, `test_execution.*`, `test_log.read`, `defect.create`/`.read`/`.update`, `test_plan.read`, `test_suite.read`, `requirement.read`, and (2026-09-15, [ADR-0075](../adr/0075-junction-table-registry-completeness.md)) `test_suite_test_case.read`/`test_plan_test_suite.read` — same reasoning as `test_manager` above — no `approval.*`, no `test_plan.approve` |
| `auditor` | `.read` on all 31 resources (29 before [ADR-0075](../adr/0075-junction-table-registry-completeness.md), 2026-09-15) + `requirement.export_rtm` — nothing else, no writes anywhere. Defined as `_read_codes(ALL_RESOURCES)`, so it gains a new read-only resource automatically |
...
**Seeded catalog (RBAC-4, ~100 rows; **102 as of [ADR-0075](../adr/0075-junction-table-registry-completeness.md)**, 2026-09-15 — migration `7d2c91af4e68`, the first RBAC extension in this repo that inserts `Permission` rows rather than only `role_permission` grants, since its two resources are new; **106 as of [ADR-0076](../adr/0076-relationship-tab-write-actions.md)**, same day — migration `3e6b08c5da71`, the second such extension, adding a `create` action to the 4 traceability link resources, see the paragraph below; **110 as of [ADR-0077](../adr/0077-relationship-tab-unlink-action.md)**, 2026-09-16 — migration `8c1d5a7b93e2`, the third, adding the matching `delete` action to the same 4, see the paragraph after that):** standard CRUD (`create`/`read`/`update`/`delete`) for 23 resources — `organization`, `org_membership`, `role`, `role_assignment`, `project`, `release`, `requirement`, `test_condition`, `test_case`, `test_step`, `test_suite`, `test_plan`, `entry_exit_criteria`, `test_cycle`, `environment`, `test_execution`, `defect`, `risk_item`, `attachment`, `test_design_technique`, `test_level`, `test_type`, `approval`; `read`-only for 8 resources — `permission`, `test_log`, `requirement_test_case_link`, `requirement_test_condition_link`, `test_condition_test_case_link`, `test_case_defect_link`, and (2026-09-15, [ADR-0075](../adr/0075-junction-table-registry-completeness.md)) `test_suite_test_case`, `test_plan_test_suite`; plus 2 special verbs — `test_plan.approve`, `requirement.export_rtm`. Seeded as an explicit, hand-authored resource list in the same Alembic data migration as the system roles (not generated from the model registry at runtime) — see [API Document](../api/2026-09-03-api-design.md) §1.

**Link-create codes ([ADR-0076](../adr/0076-relationship-tab-write-actions.md), 2026-09-15 — catalog 102 → 106, migration `3e6b08c5da71`):** the 4 [ADR-0005](../adr/0005-traceability-link-dedicated-join-tables.md) traceability link resources — `requirement_test_case_link`, `requirement_test_condition_link`, `test_condition_test_case_link`, `test_case_defect_link` — each gain a **`create` action only**, so their catalog entries go from one row (`.read`) to two. They stay in the read-only resource group rather than being promoted to full CRUD, because that group's real meaning is "no generic CRUD surface, no `update`, no `delete`" and that is still exactly true: the new `create` is one **bespoke** route each ([API Document](../api/2026-09-03-api-design.md) §4), each link entity's generic `methods` stays `{list, get}` with no create schema, and a link row remains immutable — delete-and-recreate. Promoting them into the CRUD group instead would have minted `.update`/`.delete` codes for routes that do not exist and never will. `test_suite_test_case`/`test_plan_test_suite` get **no** new code: their own link-create routes shipped under [ADR-0030](../adr/0030-req4-test-suite-membership-bespoke-routes.md)/[ADR-0031](../adr/0031-plan1-test-plan-membership-and-status-transition-routes.md) gated on the parent's `test_suite.update`/`test_plan.update`, and re-gating a live contract for symmetry would break every existing caller.

**Link-delete codes ([ADR-0077](../adr/0077-relationship-tab-unlink-action.md), 2026-09-16 — catalog 106 → 110, migration `8c1d5a7b93e2`):** the same 4 traceability link resources each gain the matching **`delete` action**, taking their catalog entries from two rows (`.read`/`.create`) to three. Declared as a **separate** `LINK_DELETE_RESOURCES` tuple rather than by widening the link-create one, even though the two hold the same four names today: they are equal by coincidence of scope, not by rule — a future junction could reasonably be append-only (create, no unlink) or delete-only (rows written solely as a side effect of another route, which is what all four of these were before ADR-0076), and one combined tuple would make either shape unrepresentable. They still stay in the read-only resource group, because the one action that group's meaning actually forbids is **`update`** — a link row is immutable (ADR-0005), and both its writes are one bespoke route each ([API Document](../api/2026-09-03-api-design.md) §4), with the generic `methods` still `{list, get}`. `test_suite_test_case`/`test_plan_test_suite` again get **no** new code: their `DELETE` routes have existed since ADR-0030/ADR-0031 gated on the parent's `test_suite.update`/`test_plan.update`, and ADR-0077 makes them *reachable from the generic surface* without re-gating them. Bundle grants: `org_admin` all 4 (its bundle is every permission that exists), `test_manager` all 4 — the role holding `requirement.export_rtm`, i.e. the one accountable for the matrix being correct rather than merely populous — and `tester` only `test_case_defect_link.delete`, the literal undo of the one link it may create. Unlike `3e6b08c5da71`, this migration's `downgrade()` is **symmetric**: it grants nothing it does not also create, so it can remove every grant and every row it made without touching anything that predates it.

Grants backfilled by the same migration: `org_admin` all 4 (its bundle is every permission that exists, by definition); `test_manager` all 4 **plus the 3 link `.read` codes it did not already hold** — read and write together, because the relationship tab these codes exist to serve gates its own list request on `.read`, so granting the write alone would ship a button on a tab that `403`s before it can render; `tester` **only** `test_case_defect_link.create` (it already holds that link's `.read` and full `defect` create/read/update); `auditor` and `ai_agent_scoped` nothing. The `downgrade()` is deliberately **asymmetric**: it deletes the 4 new `Permission` rows and every `role_permission` grant of them, but for the 3 pre-existing `.read` codes it revokes only `test_manager`'s grants and never the catalog rows, which predate this migration and other roles hold. **No schema change of any kind** — no new table, no new column, no new constraint; only `permission` and `role_permission` rows.
...
| project_id | uuid | FK → project.id, on delete restrict, **nullable** — REQ-5's standalone-authoring path, [ADR-0069](../adr/0069-req-5-standalone-test-case-with-optional-requirement-link.md). Set only by the generic factory's `create` (below); never set by either of the two pre-existing bespoke create routes, and never cleared once a standalone case later gains a `RequirementTestCaseLink` via the retrofit route (harmless historical marker — the resolver's own branch order means the more specific link always resolves first once one exists) |
...
**Generic CRUD factory posture on `TestCase`** ([ADR-0022](../adr/0022-generic-crud-router-factory.md)) — **revised 2026-09-14, [ADR-0069](../adr/0069-req-5-standalone-test-case-with-optional-requirement-link.md):** ~~no `POST /test-cases` via the factory — creation is bespoke instead~~ (superseded text, kept for history). `_TEST_CASE_CONFIG` now registers `list`+`create` via the factory, `scope_field="project_id"` — the one creation shape (REQ-5's standalone path) that needs no atomic link-table write, so it's safe to route through the plain factory unlike `TestCondition`'s own briefly-broken generic `create` (ADR-0028). The two pre-existing bespoke atomic create+link routes are unaffected: `POST /requirements/{id}/test-cases` (direct-link path, FR-REQ-2) and `POST /test-conditions/{id}/test-cases` (test-condition-mediated path, FR-REQ-3, [ADR-0028](../adr/0028-req3-test-condition-rigor-path-bespoke-routes.md)) are both still built, all three paths coexisting per-TestCase within a project (ADR-0006). `GET`/`PATCH`/`DELETE /test-cases/{id}` resolve `org_id` by walking, in order: (1) `test_condition_id` → `TestCondition.requirement_id` → `Requirement.project_id` when set; (2) if `test_condition_id IS NULL`, any linked `RequirementTestCaseLink` → `Requirement.project_id` (REQ-2's direct-link path — first-class, not an edge case, [ADR-0029](../adr/0029-testcase-resolver-direct-link-fallback.md) gap-fill); (3) `project_id` set → `Project.org_id` directly (**new, REQ-5** — a standalone case that hasn't yet gained a Requirement link; checked after both link-table branches so a case that *has* since been linked resolves via the more specific branch instead, even though its `project_id` column is still set); (4) any linked `TestSuiteTestCase` → `TestSuite.project_id`; a row satisfying none of the four (orphaned — schema-legal, but no create path in this codebase produces it) resolves to `404`, same as a genuinely missing row. `TestStep`/`Attachment` (both scoped by `test_case_id`) delegate to this same resolver.

**REQ-5's retrofit route** ([ADR-0069](../adr/0069-req-5-standalone-test-case-with-optional-requirement-link.md)): `POST /test-cases/{id}/link-requirement` inserts a `RequirementTestCaseLink` row against an *existing* `TestCase` — the same table REQ-2's own create route already writes, just decoupled from creation time. No schema change of its own (the link table already exists, §3.9 below); the only new schema surface this story adds is the `project_id` column itself, above.
