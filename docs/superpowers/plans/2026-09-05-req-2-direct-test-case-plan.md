...
- **Assumption/suggestion:** ship N independent `PATCH /test-steps/{id}` calls (AC3's "independently editable" literally asks for this shape) and accept the existing generic factory behavior — a collision surfaces as `422 validation_error`, the same posture every other entity's `update` route already has via its flush-then-catch-`IntegrityError` handler. No new reorder endpoint or renumbering logic built — out of REQ-2's AC scope, and the generic factory already behaves consistently (if not maximally convenient) on collision. A client wanting swap-without-422 should use non-adjacent sequence gaps (e.g. 10/20/30) rather than 1/2/3, or move a step to a temporary out-of-range value first. **Update, 2026-09-06:** the initial test coverage pass shipped without actually testing either half of this — TC-REQ-004 explicitly requires "reorder" tested, and it wasn't (only add + independently-edit-`action` were). Closed: `test_adjacent_sequence_swap_without_temp_value_returns_422` (proves the collision is real, not just documented) and `test_reorder_test_steps_via_temp_sequence_persists` (proves the temp-value workaround above actually reorders and persists) — see `backend/tests/integration/test_req2_direct_test_case.py`.
