...
class CreateReleaseRequest(BaseModel):
    ...
    behavior to distinguish omitted vs. explicit `null` for here (ADR-0018
...
class TestExecutionSummary(BaseModel):
    ...
    yet (checked before adding this one, per the PROJ-2 plan/ADR-0018) —
...
class TestCycleSummary(BaseModel):
    """Response item shape for `GET /releases/{id}/test-cycles` (ADR-0018 AC2).

    Nests each `TestCycle`'s `TestExecution`s directly (`executions`) rather
    than returning cycles only — ADR-0018's decision that this route answers
